    ...
    if (!preg_match("/^[a-zA-Z0-9]*$/", $username)) {
        ...
        if (!mysqli_stmt_prepare($stmt, $sql)) {
            ...
            if ($resultCheck > 0) {
                ...
                $stmt = mysqli_stmt_init($conn);
                if (!mysqli_stmt_prepare($stmt, $sql)) {
                    echo "<script>alert('SQL Error');window.location='../admin/create_hm.php'</script>";
                    exit();
                } else {
                    ...
                    if ($row = mysqli_fetch_assoc($result)) {
                        ...
                        $zz = 0;
                        $sql = "INSERT INTO Hostel_Manager (Username, Fname, Lname, Mob_no, Hostel_id, Mess_id, Pwd, Isadmin) VALUES (?, ?, ?, ?, ?, ?, ?, ?)";
                        $stmt = mysqli_stmt_init($conn);
                        if (!mysqli_stmt_prepare($stmt, $sql)) {
                            echo "<script>alert('SQL Error');window.location='../admin/create_hm.php'</script>";
                            exit();
                        } else {
                            mysqli_stmt_bind_param($stmt, "sssssssi", $username, $fname, $lname, $mobile, $HostelID, $HostelID, $hashedPwd, $zz);
